...
change. The `multilevelcoda` package implements this method in a multilevel framework and offers functions for both between- and within-person levels of variability. We discuss 4 different substitution models in this vignette.
...
For example, the significant coefficient for wilr1 shows that the within-person change in sleep behaviours (sleep duration and time awake in 
...
Here are the results for changes in stress when sleep (TST) is substituted for 5 minutes. For ease of presentation, we only show the $\pm5$ minute 
...
![plot of chunk mlmcoda-plotbsubm1](mlmcoda-mlmcoda-plotbsubm1-1.png)
...
What if we want predictions for both Males and Females? Let's do that by adding 
an additional argument \code{summary = FALSE} to `wsub()`.
...
Results for 5 minute substitution for both, separated by sex.
...
awake in bed (WAKE) for 5 minutes for both males and females, but not other behaviours. Increasing sleep at the expense of time spent awake 
...
![plot of chunk mlmcoda-plotwsubm1](mlmcoda-mlmcoda-plotwsubm1-1.png)

### Average Marginal Substitution Effects 

While `bsub()` and `wsub()` estimate the substitution model from the sample's compositional mean of 
sleep-wake behaviours, 
`bsubmargins()` and `wsubmargins()` estimate the substitution model using the average effect of all participant's compositional mean of sleep-wake behaviours. 
That is, for each participant, `bsubmargins()` and `wsubmargins()` compute the changes in the outcome 
when a fixed duration of time is reallocated from one behaviour to another, 
while the other behaviours remain constant and then average across the 
resulting effect estimates. 

Both `bsubmargins()` and `wsubmargins()`  are generally more computationally expensive than `bsub()` and `wsub()` models. All models can be run faster in  shorter walltime using parallelisation.
...
![plot of chunk mlmcoda-submargins](mlmcoda-mlmcoda-submargins-1.png)
