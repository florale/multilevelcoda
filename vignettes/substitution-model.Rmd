...
Conditional substitution models:
...
## Conditional substitution model
...
and `type = conditional` to indicate conditional substitution model.
...
This is difference from the conditional substitution model which yields prediction conditioned on 
an "average" person in the data set (by using the population- level compositional mean
as the reference composition). 
Average substitution models models are generally more computationally expensive than conditional subsitution models. All models can be run faster in shorter walltime using parallel execution. 
