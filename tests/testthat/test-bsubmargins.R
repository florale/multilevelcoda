...
expect_pairwise_direction_and_ci <- function(pair) {
  ...
  coef_estimate <- coef_summary["bz1_1", "Estimate"]
  coef_has_zero <- coef_summary["bz1_1", "Q2.5"] <= 0 &&
    0 <= coef_summary["bz1_1", "Q97.5"]
...
test_that("bsubmargin returns the expected nested result structure", {
  expect_type(x, "list")
...
test_that("bsubmargin returns estimates in a sensible range", {
  for (part in parts) {
    tmp <- stress_result(x, part)
...
test_that("bsubmargin estimates grow in magnitude from 1 to 2 minutes", {
  for (part in parts) {
    tmp <- stress_result(x, part)
    magnitude_check <- tmp[
      ,
...
for (pair in pairwise_parts) {
  test_that(
    sprintf(
      "bsubmargin matches brm for 2-component composition (%s vs %s)",
      pair[1], pair[2]
    ),
    {
