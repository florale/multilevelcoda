...
cilr <- compilr(data = mcompd[ID %in% 1:50, .SD[1:5], by = ID], sbp = sbp,
...
test_that("bsubmargins errors for invalid input", {
  ...
  ## missing substitute
  expect_error(x <- bsubmargins(object = m, minute = 2))
  
  ## not brmcoda model
  m1 <- lmer(STRESS ~ 1 + (1 | ID), data = mcompd)
  expect_error(x <- bsubmargins(object = m1, substitute = psub, minute = 2))
  
  ## not brmcoda model
  expect_error(x <- bsubmargins(object = m, substitute = psub, minute = -10))
  expect_error(x <- bsubmargins(object = m, substitute = psub, minute = 1:10))
  
  ## default minute is 60
  x1 <- bsubmargins(object = m, substitute = psub)
  x2 <- bsubmargins(object = m, substitute = psub, minute = 60)
  expect_identical(x1, x2)
  
  ## substitute has the same components as parts in cilr
  ps <- possub(c("WAKE", "MVPA", "LPA", "SB"))
  expect_error(x <- bsubmargins(object = m, substitute = ps, minute = 2))
