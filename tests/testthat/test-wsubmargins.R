...
x <- wsubmargin(object = m, base = psub, delta = 1:2)
...
expect_pairwise_direction_and_ci <- function(pair) {
  ...
  coef_estimate <- coef_summary["wz1_1", "Estimate"]
  coef_has_zero <- coef_summary["wz1_1", "Q2.5"] <= 0 &&
    0 <= coef_summary["wz1_1", "Q97.5"]
...
test_that("wsubmargin errors for missing required arguments", {
  expect_error(wsubmargin(base = psub, delta = 2))
  expect_error(wsubmargin(object = m, delta = 2))
  expect_error(wsubmargin(object = m, base = psub))
})

test_that("wsubmargin returns the expected nested result structure", {
  expect_type(x, "list")
  ...
  for (part in parts) {
    ...
    expect_true(all(tmp$Level == "within"))
...
test_that("wsubmargin returns estimates in a sensible range", {
  for (part in parts) {
    tmp <- stress_result(x, part)
...
test_that("wsubmargin estimates grow in magnitude from 1 to 2 minutes", {
  for (part in parts) {
    tmp <- stress_result(x, part)
    magnitude_check <- tmp[
      ,
...
for (pair in pairwise_parts) {
  test_that(
    sprintf(
      "wsubmargin matches brm for 2-component composition (%s vs %s)",
      pair[1], pair[2]
    ),
    {
