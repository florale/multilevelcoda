...
test_that("wsub errors for invalid input", {
  ...
  ## missing substitute
  expect_error(x <- wsub(object = m, minute = 2))
  
  ## not brmcoda model
  m1 <- lmer(STRESS ~ 1 + (1 | ID), data = mcompd)
  expect_error(x <- wsub(object = m1, substitute = psub, minute = 2))
  
  ## invalid minute
  expect_error(x <- wsub(object = m, substitute = psub, minute = -10))
  expect_error(x <- wsub(object = m, substitute = psub, minute = 1:10))
  
  ## default minute is 60
  x1 <- wsub(object = m, substitute = psub)
  x2 <- wsub(object = m, substitute = psub, minute = 60)
  expect_identical(x1, x2)
  
  ## substitute does not have the same components as parts in cilr
  ps <- possub(c("WAKE", "MVPA", "LPA", "SB"))
  expect_error(x <- wsub(object = m, substitute = ps, minute = 2))
  ...
  suppressWarnings(
    m2 <- brmcoda(compilr = cilr,
                  ...
                  backend = backend))  
  rg <- data.table(Age = 1)
  expect_warning(x <- wsub(object = m2, substitute = psub, minute = 2, regrid = rg))
...
test_that("wsub works as expected for adjusted/unadjusted model", {
  ...
  x3 <- wsub(object = m, substitute = psub, minute = 2, regrid = rg)
  ...
  x4 <- wsub(object = m, substitute = psub, minute = 2, summary = TRUE)
  x5 <- wsub(object = m, substitute = psub, minute = 2)
  expect_equal(x4, x5)
  ...
  x6 <- wsub(object = m, substitute = psub, minute = 2, summary = FALSE)
