...
bsub <- function(data, substitute, minute = 60) {
  ...
  for (i in vn) {
    ...
    names(bilr) <- c(paste0("bilr", seq_len(ncol(bilr))))
    names(tilr) <- c(paste0("bilr", seq_len(ncol(tilr))))
    names(wilr) <- c(paste0("wilr", seq_len(ncol(wilr))))
