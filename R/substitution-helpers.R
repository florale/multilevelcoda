...
#'
create_substitution <-
  function(BetweenSub, WithinSub, Sub,
           BetweenSubMargins, WithinSubMargins, SubMargins,
           delta,
           ref,
           level,
           weight,
...
build.rg <- function(object,
  ...
  if (model_ranef_level == "multilevel") {
    ...
    if (level == "combined") {
      comp0 <- object$CompILR$Comp
      d0 <- cbind(object$CompILR$Comp, object$CompILR$data)
      d0 <- d0[, head(.SD, 1), by = eval(object$CompILR$idvar)]
      comp0 <- acomp(d0[, colnames(object$CompILR$Comp), with = FALSE], total = object$CompILR$total)
    }
...
.get.sub <- function(object, delta, basesub,
  ...
  iout <- foreach(i = colnames(basesub), .combine = c, 
                  .options.future = list(packages = "multilevelcoda")) %dofuture% {
                    ...
                    # compositions and ilrs for predictions
                    comp0 <- acomp(dnew[, colnames(object$CompILR$Comp), with = FALSE], total = object$CompILR$total)
                    compsub  <- acomp(dnew[, object$CompILR$parts, with = FALSE], total = object$CompILR$total)
                    
                    ilrsub <- ilr(bcompsub, V = object$CompILR$psi)
                    colnames(ilrsub) <- colnames(object$CompILR$ILR)
                    
                    # reference grid 
                    ## get covariate + idvar names
                    covnames <- colnames(d0) %snin% c(colnames(object$CompILR$ILR),
                                                      colnames(object$CompILR$Comp)
                    )
                    refgrid <- d0[, covnames, with = FALSE]
                  ...
                  .options.future = list(packages = "multilevelcoda")) %dofuture% {    
                    ...
                    for (j in seq_along(delta)) { # delta level
                      ...
                      for (k in seq_len(nrow(sub))) {
                        ...
                        colnames(tilr) <- colnames(object$CompILR$ILR)
