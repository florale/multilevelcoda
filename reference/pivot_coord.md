...
This function estimates pivot balance coordinates for each compositional
part by either `"rotate"` the sequential binary partition using the same
`brmcoda` object or `"refit"` the `brmcoda` object.
...
- method:
  ...
  A character string. Should the pivot balance coordinates be estimated
  by `"rotate"` the sequential binary partition using the same `brmcoda`
  object or `"refit"` the `brmcoda` object? Default is `"rotate"`.
